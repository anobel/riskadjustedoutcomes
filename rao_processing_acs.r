...
# This can be generated from raw data in rao_processing_zip.r
...
acsd$ethnicity <- acs.fetch(geo_zip,
                            table.number = "B03002",
                            endyear = 2011,
                            span = 5,
                            col.names = "pretty")
...
# Calculate summary variables for each ethnicity
acsd$ethwhite <- divide.acs(num = acsd$ethnicity[, 3],
                              den = acsd$ethnicity[, 1],
                              method = "proportion")
acsd$ethblack <- divide.acs(num = acsd$ethnicity[, 4],
                              den = acsd$ethnicity[, 1],
                              method = "proportion")
acsd$ethlatin <- divide.acs(num = acsd$ethnicity[, 12],
                              den = acsd$ethnicity[, 1],
                              method = "proportion")
acsd$ethasian <- divide.acs(num = acsd$ethnicity[, 6],
                              den = acsd$ethnicity[, 1],
                              method = "proportion")
acsd$ethother <- apply(X = acsd$ethnicity[, c(5, 7:11)], 2, 
                      FUN = sum, agg.term = "ethother")
acsd$ethother <- divide.acs(num = acsd$ethother,
                              den = acsd$ethnicity[, 1],
                              method = "proportion")

# B19013 - Median Household Income
acsd$medianincome <- acs.fetch(geo_zip,
                                table.number = "B19013",
                                endyear = 2011,
                                span=5,
                                col.names = "pretty")
...
acsd$incomegroups <- acs.fetch(geo_zip,
                              table.number = "B19001",
                              endyear = 2011,
                              span = 5,
                              col.names = "pretty")
...
acsd$incomeunder25k <- apply(X = acsd$incomegroups[, c(2:5)], 2,
                            FUN = sum, agg.term = "incomeunder25k")
acsd$incomeunder25k <- divide.acs(num = acsd$incomeunder25k,
                                  den = acsd$incomegroups[, 1],
                                  method = "proportion")
...
acsd$incomeover100k <- apply(X = acsd$incomegroups[, c(14:17)], 2,
                            FUN = sum, agg.term = "incomeover100k")
acsd$incomeover100k <- divide.acs(num = acsd$incomeover100k,
                                  den = acsd$incomegroups[, 1],
                                  method = "proportion")

# B19054 - Interest, Dividends, or Net Rental Income for Households 
acsd$capincome <- acs.fetch(geo_zip,
                            table.number = "B19054",
                            endyear = 2011,
                            span = 5,
                            col.names = "pretty")
...
acsd$capincome <- divide.acs(num = acsd$capincome[, 2],
                             den = acsd$capincome[, 1],
                             method = "proportion")

# B25077 - Median Value (Dollars) for Owner-Occupied Housing Units
acsd$housevalue <- acs.fetch(geo_zip,
                             table.number = "B25077",
                             endyear = 2011,
                             span = 5,
                             col.names = "pretty")
...
acsd$edunohs <- apply(X = acsd$edu[, c(3:10, 20:27)], 2,
                      FUN = sum, agg.term = "nohs")

# divide by total ZCTA population (using ACS functions) to get proportion
acsd$edunohs <- divide.acs(num = acsd$edunohs,
                           den = acsd$edu[, 1],
                           method = "proportion")
...
acsd$eduhs <- apply(X = acsd$edu[, c(11:18, 28:35)], 2,
                    FUN = sum, agg.term = "hs")

# divide by total ZCTA population (using ACS functions) to get proportion
acsd$eduhs <- divide.acs(num = acsd$eduhs,
                         den=acsd$edu[, 1],
                         method = "proportion")

# For numbers that completed college, combined the last 4 columns above
acsd$educollege <- apply(X = acsd$edu[, c(15:18, 32:35)], 2,
                         FUN = sum, agg.term = "college")

# divide by total ZCTA population (using ACS functions)
acsd$educollege <- divide.acs(num = acsd$educollege,
                              den = acsd$edu[, 1],
                              method = "proportion")

# C24010 Sex by Occupation for the Civilian Employed Population 16 years and over
acsd$emp <- acs.fetch(geo_zip,
                      table.number = "C24010",
                      endyear = 2011,
                      span = 5,
                      col.names = "pretty")
...
acsd$empexec <- apply(X = acsd$emp[, c(3, 39)], 2,
                      FUN = sum, agg.term = "exec")
acsd$empexec <- divide.acs(num = acsd$empexec,
                           den = acsd$pop,
                           method="proportion")

# B19083 Gini Index Of Income Inequality
acsd$gini <- acs.fetch(geo_zip,
                       table.number = "B19083",
                       endyear = 2011,
                       span = 5,
                       col.names = "pretty")
...
acsz <- as.data.frame(cbind(zcta = acs[, 1],
                           scale(acs[, c(2:length(acs))], center = T, scale = T)
                          )
)
...
acs$hoodzscore <- rowSums(cbind(acsz$medianincomelog,
                                acsz$housevalue,
                                acsz$capincome,
                                acsz$eduhs,
                                acsz$educollege,
                                acsz$empexec), na.rm = T)
...
acs <- acs %>%
  select(zcta, pop, pop10k,
         ...
         medianincome, medianincome1k, incomeunder25k, incomeover100k,
         capincome,
...
# Save into CSV
write.csv(acs, file="data/tidy/acs.csv", row.names=F)