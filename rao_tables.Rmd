...
t1 <- ptgu %>%
  ...
  dplyr::summarise(
    ...
    "Urology Residency Affiliation (%)" = sprintf("%.1f %%", 100*sum(residencygu, na.rm=T)/n()),
...
t1p["Urology Residency Affiliation (%)",1] <- format.pval(
...
t2p <- ptgu %>%
  ...
  do(
    ...
    "Urology Residency Affiliation (%)" = chisq.test(.$residencygu, .$isreadmit30dc)$p.value,
...
colnames(t2p) <- paste(t2pcols, "p-value", sep="_")
t2p$id <- t2prows
