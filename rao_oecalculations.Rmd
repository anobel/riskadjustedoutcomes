...
```{r}
perm <- pred %>% 
    mutate(dsh = factor(ifelse(safetynaph, "Yes", "No")))%>%
    mutate(
      changerank_base_v_obs = r_base - r_obs,
      changerank_base_ses_v_obs = r_base_ses - r_obs,
    ...
    group_by(cohort) %>%
    do(results = 
      coin::independence_test(changerank_base_v_base_ses ~ dsh, data = ., ytrafo = rank_trafo, distribution = exact())
    )

# list just the p-values from the permutation testing
lapply(perm$results, pvalue)
