...
pred <- pred_all %>%
  ...
  group_by(cohort) %>%
  mutate_each_(funs(rank(., ties.method="average")), vars) %>%
  right_join(ranks)
...
independence_test(changerank_base_v_base_ses ~ dsh, data = oedf, ytrafo = rank_trafo, distribution = exact())

oedf$dsh[oedf$safetydsh] <- "Yes"
oedf$dsh[!oedf$safetydsh] <- "No"
oedf$dsh <- factor(oedf$dsh)


independence_test(time ~ group, data = rotarod, ytrafo = rank_trafo, distribution = exact())
